...
class STACKED_TRANSFORMS(object):
    """Transform the ECT matrix by randomly rotating the input image (randomly select a rotation angle) and translating the columns of the ECT image accordingly.

    transforms.Compose([transforms.ToTensor(),transforms.Normalize(0.5,0.5),transforms.RandomRotation((0,360), fill=fill)])

    transforms.Compose([transforms.ToTensor(),transforms.Normalize(0.5,0.5), ROTATE_TRANSLATE()])
    """

    def __call__(self, sample):

        # transform all channels to tensor and normalize
        transform_tensor_norm = transforms.Compose([transforms.ToTensor(),transforms.Normalize(0.5,0.5)])
        sample = transform_tensor_norm(sample)
        sample = sample.permute((1, 2, 0)).contiguous()

        # get random rotation angle, use to determine how many columns to shift
        r_angle = random.uniform(0, 2*np.pi)

        #Translate columns of ECT according to the random angle
        ect_sample = sample[1:,:,:]
        cols = np.linspace(0,2*np.pi, 32)
        col_idx = min(range(len(cols)), key=lambda i: abs(cols[i]-r_angle))
        first = ect_sample[:,:,col_idx:]
        second = ect_sample[:,:,0:col_idx]
        ect_sample = torch.concatenate((first,second), axis=2)

        #Rotate image according to random angle
        im_sample = sample[0,:,:].unsqueeze(0)
        dataset='leafgraph'
        if dataset=='mpeg7':
            fill = -1
        else:
            fill = 1
        transform_rotate = transforms.Compose([transforms.RandomRotation((r_angle,r_angle), fill=fill)])
        im_sample = transform_rotate(im_sample)

        new_sample = torch.concatenate((im_sample, ect_sample), axis=0)
        return new_sample
